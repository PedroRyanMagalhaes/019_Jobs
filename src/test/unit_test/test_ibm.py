"""
Teste unitário para o scraper da IBM
"""
import sys
...
def test_scraper():
    ...
    print(f"--- TESTANDO SCRAPER DA IBM ---")
    
    # Inicializa banco e limpa vagas anteriores
    database.inicializar_banco()
    conn = database.sqlite3.connect(TEST_DB_FILE)
    conn.execute("DELETE FROM vagas WHERE empresa = 'IBM'")
    conn.commit()
    conn.close()
    
    # Executa o scraper
    vagas_coletadas = Ibm.raspar()

    # Assert: deve coletar pelo menos uma vaga
    assert vagas_coletadas, "❌ Nenhuma vaga da IBM foi encontrada no teste."
    assert len(vagas_coletadas) > 0, "Lista de vagas está vazia"
    
    print(f"\n✅ SUCESSO! {len(vagas_coletadas)} vagas da IBM encontradas.")
