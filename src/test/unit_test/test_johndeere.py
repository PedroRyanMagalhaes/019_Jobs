"""
Teste unitário para o scraper da John Deere
"""
import sys
...
def test_scraper():
    ...
    print(f"--- TESTANDO SCRAPER DA JOHN DEERE ---")
    ...
    # Executa o scraper
    vagas_coletadas = Johndeere.raspar()

    # Assert: deve coletar pelo menos uma vaga
    assert vagas_coletadas, "❌ Nenhuma vaga da John Deere foi encontrada no teste."
    assert len(vagas_coletadas) > 0, "Lista de vagas está vazia"
    
    print(f"\n✅ SUCESSO! {len(vagas_coletadas)} vagas da John Deere encontradas.")
