"""
Teste unitário para o scraper do iFood
"""
import sys
...
def test_scraper():
    ...
    print(f"--- TESTANDO SCRAPER DO IFOOD ---")
    ...
    # Executa o scraper
    vagas_coletadas = Ifood.raspar()

    # Assert: deve coletar pelo menos uma vaga
    assert vagas_coletadas, "❌ Nenhuma vaga do iFood foi encontrada no teste."
    assert len(vagas_coletadas) > 0, "Lista de vagas está vazia"
    
    print(f"\n✅ SUCESSO! {len(vagas_coletadas)} vagas do iFood encontradas.")
