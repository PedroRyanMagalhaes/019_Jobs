"""
Teste unitário para o scraper da CPFL
"""
import sys
...
def test_scraper():
    ...
    print(f"--- TESTANDO SCRAPER DA CPFL ---")
    ...
    # Executa o scraper
    vagas_coletadas = Cpfl.raspar()

    # Assert: deve coletar pelo menos uma vaga
    assert vagas_coletadas, "❌ Nenhuma vaga da CPFL foi encontrada no teste."
    assert len(vagas_coletadas) > 0, "Lista de vagas está vazia"
    
    print(f"\n✅ SUCESSO! {len(vagas_coletadas)} vagas da CPFL encontradas.")
