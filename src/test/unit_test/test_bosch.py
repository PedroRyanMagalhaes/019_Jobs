"""
Teste unitário para o scraper da Bosch
"""
import sys
...
def test_scraper():
    ...
    print(f"--- TESTANDO SCRAPER DA BOSCH ---")
    
    # Inicializa banco e limpa vagas anteriores
    database.inicializar_banco()
    conn = database.sqlite3.connect(TEST_DB_FILE)
    conn.execute("DELETE FROM vagas WHERE empresa = 'Bosch'")
    conn.commit()
    conn.close()
    
    # Executa o scraper
    vagas_coletadas = Bosch.raspar()

    # Assert: deve coletar pelo menos uma vaga
    assert vagas_coletadas, "❌ Nenhuma vaga da Bosch foi encontrada no teste."
