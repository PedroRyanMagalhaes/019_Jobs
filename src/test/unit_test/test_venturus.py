...
def test_scraper():
    ...
    print(f"--- TESTANDO SCRAPER DA VENTURUS ---")
    ...
    assert vagas_coletadas, "❌ Nenhuma vaga da Venturus foi encontrada no teste."
    assert len(vagas_coletadas) > 0, "Lista de vagas está vazia"
    
    print(f"\n✅ SUCESSO! {len(vagas_coletadas)} vagas da Venturus encontradas.")
