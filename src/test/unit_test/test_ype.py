"""
Teste unitário para o scraper da Ypê
"""
import sys
...
def test_scraper():
    ...
    print(f"--- TESTANDO SCRAPER DA YPÊ ---")
    
    # Inicializa banco e limpa vagas anteriores
    database.inicializar_banco()
    conn = database.sqlite3.connect(TEST_DB_FILE)
    conn.execute("DELETE FROM vagas WHERE empresa = 'Ypê'")
    conn.commit()
    conn.close()
    
    # Executa o scraper
    vagas_coletadas = Ype.raspar()

    # Assert: deve coletar pelo menos uma vaga
    assert vagas_coletadas, "❌ Nenhuma vaga da Ypê foi encontrada no teste."
    assert len(vagas_coletadas) > 0, "Lista de vagas está vazia"
    
    print(f"\n✅ SUCESSO! {len(vagas_coletadas)} vagas da Ypê encontradas.")
