"""
Teste unitário para o scraper da CI&T
"""
import sys
...
def test_scraper():
    ...
    print(f"--- TESTANDO SCRAPER DA CI&T ---")
    ...
    # Executa o scraper
    vagas_coletadas = Ciandt.raspar()

    # Assert: deve coletar pelo menos uma vaga
    assert vagas_coletadas, "❌ Nenhuma vaga da CI&T foi encontrada no teste."
    assert len(vagas_coletadas) > 0, "Lista de vagas está vazia"
    
    print(f"\n✅ SUCESSO! {len(vagas_coletadas)} vagas da CI&T encontradas.")
