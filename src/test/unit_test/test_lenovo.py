"""
Teste unitário para o scraper da Lenovo
"""
import sys
...
def test_scraper():
    ...
    print(f"--- TESTANDO SCRAPER DA LENOVO ---")
    ...
    # Executa o scraper
    vagas_coletadas = Lenovo.raspar()

    # Assert: deve coletar pelo menos uma vaga
    assert vagas_coletadas, "❌ Nenhuma vaga da Lenovo foi encontrada no teste."
    assert len(vagas_coletadas) > 0, "Lista de vagas está vazia"
    
    print(f"\n✅ SUCESSO! {len(vagas_coletadas)} vagas da Lenovo encontradas.")
