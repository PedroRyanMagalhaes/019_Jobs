"""
Teste unitário para o scraper da Kumulus
"""
import sys
...
def test_scraper():
    ...
    print(f"--- TESTANDO SCRAPER DA KUMULUS ---")
    ...
    # Executa o scraper
    vagas_coletadas = Kumulus.raspar()

    # Assert: deve coletar pelo menos uma vaga
    assert vagas_coletadas, "❌ Nenhuma vaga da Kumulus foi encontrada no teste."
    assert len(vagas_coletadas) > 0, "Lista de vagas está vazia"
    
    print(f"\n✅ SUCESSO! {len(vagas_coletadas)} vagas da Kumulus encontradas.")
