...
def raspar():
    """
    Realiza o scraping completo das vagas da Honda (Gupy).
    - Aceita cookies
    - Filtra por SP
    - Busca vagas em Sumaré - SP (com ou sem "and Hybrid")
    - Marca todas como Híbrido
    """
    
    url = EMPRESA_URLS.get("Honda")
    if not url:
        print("❌ URL da Honda não encontrada nas configurações.")
        return []

    vagas_para_salvar = []
    base_url = "https://honda.gupy.io" 

    print(f"Iniciando scraper para a Honda em {url} (Forçando idioma Inglês)")
        ...
        try:
            ...
            while True:
                ...
                for item in vaga_items:
                    ...
                    # Filtra por Sumaré - SP (com ou sem "and Hybrid")
                    if 'sumaré' in local_lower and 'sp' in local_lower:
                        
                        url_relativa = link_tag.get_attribute('href')
                        url_vaga = f"{base_url}{url_relativa}"
                        
                        # Todas as vagas de Sumaré são marcadas como Híbrido
                        modelo = "Híbrido"
            ...
            print(f"❌ Ocorreu um erro crítico no scraper da Honda: {e}")
        finally:
...
if __name__ == "__main__":
    ...
    print(f"--- EXECUTANDO SCRAPER DA HONDA EM MODO DE TESTE (EM INGLÊS) ---")
    ...
    if vagas_coletadas:
        print(f"\n✅ SUCESSO! {len(vagas_coletadas)} vagas da Honda selecionadas.")
        ...
        print("\n❌ Nenhuma vaga da Honda foi selecionada no teste.")
